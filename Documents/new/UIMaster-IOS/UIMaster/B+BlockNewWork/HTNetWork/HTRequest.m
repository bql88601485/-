...
+ (instancetype)sharedRequestWithMethodType:(RequestMethod)methodType withPath:(NSString *)path withParam:(NSDictionary *)paramDic withConfig:(HTRequestConfig *)config respondData:(id)searchNetResult{
    return [[self alloc]initWithMethodType:methodType WithPath:path withParam:paramDic withConfig:config withData:nil respondData:searchNetResult];
}
...
+ (instancetype)sharedRequestWithData:(NSData *)data withPath:(NSString *)path withParam:(NSDictionary *)paramDic withConfig:(HTRequestConfig *)config{
    return [[self alloc]initWithMethodType:RequestMethod_Post WithPath:path withParam:paramDic withConfig:config withData:data respondData:nil];
}

- (instancetype)initWithMethodType:(RequestMethod)methodType WithPath:(NSString *)path withParam:(NSDictionary *)paramDic withConfig:(HTRequestConfig *)config withData:(NSData *)data respondData:(id)searchNetResult{
    self = [super init];
    if (!self) {
        return nil;
    }
    ...
    _tag = @"0";
    _searchNetResult = searchNetResult;
