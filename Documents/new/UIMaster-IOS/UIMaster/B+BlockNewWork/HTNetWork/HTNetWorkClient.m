...
- (instancetype)init {
    ...
    NSLog(@"headfile : %@",_requestManager.requestSerializer.HTTPRequestHeaders);
...
- (void)startAsynchronous:(HTRequest *)request{
    ...
    if (request.methodType ==RequestMethod_Get) {
        NSURLSessionDataTask *task =[_requestManager GET:[request.url absoluteString] parameters:request.parameters  progress:nil
...
- (void)startAsynchronous:(HTRequest*)request
    ...
    if (request.methodType ==RequestMethod_Get) {
        NSURLSessionDataTask *task = [_requestManager GET:[request.url absoluteString] parameters:request.parameters
                                                 progress:^(NSProgress *progress){
                                                     if (progressBlock) {
        ...
        [self addCurrentRequest:request withTask:task];
    }else if(request.methodType ==RequestMethod_Put){
        
        NSURLSessionDataTask *task = [_requestManager PUT:[request.url absoluteString] parameters:request.parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            NSLog(@"responseObject: %@ \n",responseObject);
...
- (void)onResponseWithFinishBlock:(HITResponseObject)responseBlock
    ...
    data = [request.searchNetResult modelWithJSON:data];
    ...
    if (netWorkError) {
        ...
        if (self.defaultUseCache) {
            ...
            for (NSString *key in [request.parameters allKeys]) {
                ...
                NSString *value = [request.parameters objectForKey:key];
            ...
            [[HTDataCache sharedManager] cacheDataWithUrl:fullPath withData:data];
        }
...
- (id)getJsonDataWithString:(NSString *)jsonStr withError:(HTNetWorkError **)dataError{
    ...
    if (error || ![jsonDic objectForKey:@"code"]
        || [[jsonDic objectForKey:@"code"] isKindOfClass:[NSNull class]]) {
        *dataError =[[HTNetWorkError alloc]init];
        HTNetWorkError *networkError = *dataError;
        networkError.errorCode = -2;
        networkError.errorDescription = @"数据访问错误";
        return nil;
    }else if ([[jsonDic objectForKey:@"code"] intValue] !=200){
        *dataError =[[HTNetWorkError alloc]init];
        HTNetWorkError *networkError = *dataError;
