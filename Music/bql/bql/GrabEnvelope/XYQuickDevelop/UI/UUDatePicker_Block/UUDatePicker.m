...
- (void)drawRect:(CGRect)rect
{
    ...
    cancel.frame = CGRectMake(0, 0, 50, 20);
    cancel.backgroundColor   = [UIColor colorWithRed:0.000 green:0.502 blue:1.000 alpha:1.000];
    __weak UUDatePicker *weakSelf = self;
    [cancel addTapGestureWithBlock:^(UIView *view) {
        [weakSelf canceMyview];
    }];
    [cancel setTitle:@"取消" forState:UIControlStateNormal];
    [self addSubview:cancel];
    
    UIButton *ok = [UIButton buttonWithType:UIButtonTypeCustom];
    ok.frame = CGRectMake(self.frame.size.width - 50, 0, 50, 20);
    ok.backgroundColor   = [UIColor colorWithRed:0.000 green:0.502 blue:1.000 alpha:1.000];
    [ok addTapGestureWithBlock:^(UIView *view) {
        [weakSelf okSelectMyview];
    }];
...
- (NSArray *)getNowDate:(NSDate *)date
{
    ...
    if (self.datePickerStyle == UUDateStyle_HourMinute)
        return @[hour,minute];
    if (self.datePickerStyle == UUDateStyle_Year) {
        return @[year];
    }
    return nil;
}
